...
- Serena project config enables `typescript` and `cpp`. `var/sunvox_lib/` is intentionally analyzable for SunVox C/C++ source work; root `/sunvox_lib/` runtime files stay ignored.
- On Windows, Serena CLI commands that start language servers need UTF-8 output (`$env:PYTHONIOENCODING='utf-8'`) and may require running outside the sandbox to install/start clangd under `C:\Users\mande\.serena\language_servers\`.

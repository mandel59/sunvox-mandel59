...
`.sunsynth` outputs. Recipe files describe synth creation and editing with
plain JavaScript recipe files that need no runtime imports. Recipes can still
use `apply(editor)` for procedural edits; the MVP exposes `SunSynthEditor` and
`SunVoxProjectEditor` facades backed by the existing SunSynth lab
implementation. Type inference comes from `tools/sunvox-edit-recipe.d.ts`. The
...
`sunvox:music-recipe` applies SunVox Music Recipe files to create `.sunvox`
project outputs from structured documents built with the codec. Use it for
generated music and BGM workflows that should be reproducible without ad hoc
branch-local scripts. Type inference comes from
`tools/sunvox-music-recipe.d.ts`; outputs can also request a JSON summary with
the source recipe, issue/tags, generated byte size, project counts, and
validation result. Keep research outputs and summaries under `var/`; keep
human-authored research recipes under `recipes/research/`; put promoted music
recipes under `generated/recipes/music/` with reviewed outputs under
`generated/music/`. Generated site data records source recipes for generated
music projects from `generated/recipes/music/`, while generated instruments
continue to use `generated/recipes/sunvox-edit/`.
...
- `generated/recipes/sunvox-edit/`: committed `.sunsynth` recipes.
- `generated/recipes/music/`: committed `.sunvox` music recipes.
